...
| --------------------- | ---------------------------- |
| macOS (Apple Silicon) | `fs-scan_mac_arm64`          |
| macOS (Intel)         | `fs-scan_mac_amd64`          |
| Linux (x86_64)        | `fs-scan_linux_amd64`        |
| Linux (ARM64)         | `fs-scan_linux_arm64`        |
| Windows (x86_64)      | `fs-scan_windows_amd64.exe`  |
| Windows (ARM64)       | `fs-scan_windows_arm64.exe`  |


Each binary also has a corresponding `.sig` (signature) and `.pem`
(certificate) file in the release, along with a `fs-scan_checksums.txt` file
containing SHA-256 hashes of all binaries. See
[Verifying the Binary](#2-verify-the-binary-optional) below.

Download the binary for your platform, rename it to `fs-scan` (or `fs-scan.exe` on
Windows), and make it executable:

```bash
mv fs-scan_mac_arm64 fs-scan
chmod +x fs-scan
```
...
```bash
xattr -d com.apple.quarantine ./fs-scan
```
...
| File                     | Purpose                                          |
| ------------------------ | ------------------------------------------------ |
| `fs-scan_mac_arm64.sig`  | Cosign signature for the binary                  |
| `fs-scan_mac_arm64.pem`  | Signing certificate (contains the OIDC identity) |


There is also a `fs-scan_checksums.txt` (with its own `.sig` and `.pem`) that
...
```bash
cosign verify-blob \
  --signature fs-scan_mac_arm64.sig \
  --certificate fs-scan_mac_arm64.pem \
...
cosign verify-blob \
  --signature fs-scan_checksums.txt.sig \
  --certificate fs-scan_checksums.txt.pem \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
  --certificate-identity-regexp "github.com/FiniteStateInc/" \
  fs-scan_checksums.txt

# 2. Check your binary against the verified checksums
sha256sum -c fs-scan_checksums.txt --ignore-missing
```
...
```bash
./fs-scan --dir /path/to/project --output bom.cdx.json
```
...
- **HTML** — a visual report you can open in a browser. Useful for reviewing
  results by hand.
  ```bash
  ./fs-scan --dir /path/to/project --format html
  ```
- **JSON** (`simple`) — raw JSON to stdout. Useful for scripting or piping
  into tools like `jq`.
  ```bash
  ./fs-scan --dir /path/to/project --format simple
  ```
...
The tool combines six independent detection methods into a single scan. Each
one targets a different class of software component. All six run automatically
...
Extracts direct dependencies from manifest files across all major ecosystems:
npm (`package.json`), Maven (`pom.xml`), Gradle (`build.gradle`), sbt
(`build.sbt`), Go (`go.mod`), Cargo (`Cargo.toml`), Python (`pyproject.toml`,
`requirements.txt`), NuGet (`.csproj`), Ruby (`Gemfile`), and Composer
(`composer.json`). Native build tool invocation is enabled by default; use
`--invoke-native-tools=false` to disable external tool execution and stick to static parsing.
...
When a directory tree contains multiple projects (or multiple manifest files
that list the same packages), the scanner automatically deduplicates components
by their Package URL (PURL). Locations and licenses from duplicates are merged
so no information is lost, while the final count reflects truly unique packages.
...
```bash
./fs-scan ... --api-token "your-api-token-here"
```
...
`vendor/`, `.git/`, and .NET build artifacts (`project.assets.json`,
`*.deps.json`) from scanning. The `.fsignore` file lets you add
project-specific exclusions on top of those defaults.
...
```bash
./fs-scan --dump-embedded-config config.yaml
```

Use `-` instead of a filename to print it to stdout:

```bash
./fs-scan --dump-embedded-config -
```
...
exclude_paths:
  ...
  - "**/vendor/**"
  - "**/.git/**"
  - "**/project.assets.json"   # NuGet build artifacts (dotnet restore)
  - "**/*.deps.json"           # .NET runtime dependency manifests
...
The `--top-level` flag gives you a focused, actionable view of your
project's dependencies by applying three filters:

1. **Manifest parser disabled**: The Syft-based manifest parser is turned
   off entirely. Its lockfile scanning produces massive transitive
   dependency noise, and its non-lockfile results largely duplicate what
   the build-tool detector already provides. Disabling it also makes scans
   significantly faster.
2. **Direct dependencies only**: The build-tool detector parses manifest
   files (`package.json`, `Cargo.toml`, `go.mod`, `pom.xml`, `.csproj`,
   `pyproject.toml`, `Gemfile`, `composer.json`, etc.) to report only
   the dependencies you directly declared. Transitive detections
   (e.g. `// indirect` requires in `go.mod`) are filtered out.
3. **Ocean rules**: When a rule marked `top_level: true` matches a
   directory, nested detections under that directory are suppressed —
   only the top-level component is reported.

This typically reduces component counts by ~90% compared to a full scan,
with no loss of actionable information for product security.

```bash
./fs-scan --dir /path/to/project --top-level --output bom.cdx.json
```
...
- You're seeing too much noise from transitive dependencies (thousands of
  packages pulled in through lockfiles) and want only what you directly
  depend on.
- You want a focused view for vulnerability triage — direct dependencies
  are what you can actually upgrade.

**When to leave it off (the default):**

- You need full visibility into every individual library, including all
  transitive dependencies resolved from lockfiles.
- You're doing deep vulnerability analysis and need the complete dependency
  graph.
...
```bash
./fs-scan --dir /path/to/project --rules /path/to/rules
```
...
```bash
./fs-scan --dir /path/to/project --config my-config.yaml
```
...
```bash
./fs-scan --dir /path/to/project --feedback --output project.cdx.json
```
...
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Quick scan (auto-saves CycloneDX) | `./fs-scan --dir /path`                                                                                                             |
| CycloneDX SBOM to specific file   | `./fs-scan --dir /path --output bom.cdx.json`                                                                                       |
| Top-level components only         | `./fs-scan --dir /path --top-level --output bom.cdx.json`                                                                           |
| Upload to Finite State            | `./fs-scan --dir /path --api-url URL --project-name NAME --project-version VER`                                                     |
| Upload via env vars               | `export FINITE_STATE_DOMAIN=... FINITE_STATE_AUTH_TOKEN=...` then `./fs-scan --dir /path --project-name NAME --project-version VER` |
| Upload + save locally             | `... --api-url URL --project-name NAME --project-version VER --output bom.cdx.json`                                             |
| Export embedded config            | `./fs-scan --dump-embedded-config config.yaml`                                                                                      |
| Use a custom config               | `./fs-scan --dir /path --config my-config.yaml`                                                                                     |
| Use external rules directory      | `./fs-scan --dir /path --rules /path/to/rules`                                                                                      |
| Feedback on scan results          | `./fs-scan --dir /path --feedback --output bom.cdx.json`                                                                            |
| HTML report (debug)               | `./fs-scan --dir /path --format html`                                                                                               |
| JSON to stdout (debug)            | `./fs-scan --dir /path --format simple`                                                                                             |
| See all options                   | `./fs-scan --help`                                                                                                                  |
