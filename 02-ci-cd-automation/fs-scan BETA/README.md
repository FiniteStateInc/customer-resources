...
| **Build Tool** | Extracts direct dependencies from manifest files: npm (`package.json`), Maven (`pom.xml`), Gradle, sbt, Go (`go.mod`), Cargo (`Cargo.toml`), Python (`pyproject.toml`, `requirements.txt`), NuGet (`.csproj`), Ruby (`Gemfile`), Composer (`composer.json`). Native build tool invocation is enabled by default; set `--invoke-native-tools=false` to disable it. |
...
./fs-scan --dir /path/to/project --rules ../ocean/rules

# HTML report (auto-saved to <dirname>-sbom.html)
./fs-scan --dir /path/to/project --rules ../ocean/rules --format html

# JSON to stdout (debug)
./fs-scan --dir /path/to/project --rules ../ocean/rules --format simple

# Full JSON with all metadata
./fs-scan --dir /path/to/project --rules ../ocean/rules --format full

# Save to a specific file
./fs-scan --dir /path/to/project --rules ../ocean/rules --format html --output report.html

# Feedback UI (opens a local browser UI; auto-selects CycloneDX output)
./fs-scan --dir /path/to/project --rules ../ocean/rules --feedback --output project.cdx.json

# Upload CycloneDX SBOM to Finite State URL (uses FINITE_STATE_AUTH_TOKEN by default)
FINITE_STATE_AUTH_TOKEN=... ./fs-scan --dir /path/to/project --rules ../ocean/rules \
...
> may block execution on first launch. Run `xattr -d com.apple.quarantine ./fs-scan`
...
| `--top-level` | Top-level mode: disable manifest parser (Syft), suppress nested Ocean detections, and report only direct dependencies via the build-tool detector (overrides config `top_level_only`). Typically reduces output by ~90%. | `false` |
...
| `--version` | Print fs-scan build version and exit | `false` |
...
| `--transitive` | Include transitive dependencies in output; set `--transitive=false` for direct-only output across detectors | `true` |
| `--invoke-native-tools` | Invoke native build tools (mvn, gradle, sbt, go) during dependency resolution; set `--invoke-native-tools=false` to disable | `true` |
...
When scanning large directory trees — especially collections of multiple projects — the same package often appears in many lockfiles and manifests (e.g. `wrappy@1.0.2` in dozens of `package-lock.json` files, or `Newtonsoft.Json@13.0.3` in scores of NuGet `project.assets.json` files). The scanner automatically deduplicates components after all detectors finish:

- Two detections are considered duplicates when they share the same **PURL** (Package URL). For detections without a PURL, the key is `type/name@version`.
- The **first occurrence** is kept; locations and licenses from later duplicates are merged into it so no signal is lost.
- Deduplication runs before output formatting, so summary counts, CycloneDX, HTML, and JSON outputs all reflect deduplicated totals.

In practice this eliminates 50–70% of raw detections on multi-project trees with no loss of meaningful data. For even greater reduction, use `--top-level` to disable the manifest parser entirely and report only direct dependencies (~90% reduction).
...
│   │   ├── buildtool_detector.go  # Build-tool dependency resolution (Maven, Gradle, sbt, Go)
